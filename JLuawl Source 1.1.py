    ...
    def __init__(self, master,table_list, **kwargs):
        ...
        try:
            for i,v in enumerate(table_list):
             ...
             NotificationWindow(text=f'Keys Error: {e} (Is your API Token Valid? / Server Connection Error)',nclass='Warning')
...
class ToplevelWindow(ctk.CTkToplevel):
    def __init__(self, whitelist, *args, **kwargs):
        ...
        headers = {
            'Authorization': 'Bot MTEwNTU5MjYzMDI2MTMzNDA3Ng.GRAHIf.c5WvFABbSp43weUn37TTLnIzN_diJmZOnN95_c',
            'User-Agent': 'MyDiscordBot/1.0'
        }
        req = urllib.request.Request(ThumbURL, headers=headers)
        try:
            with urllib.request.urlopen(req) as u:
                raw_data = u.read()
                im = Image.open(io.BytesIO(raw_data))
                my_image = ctk.CTkImage(light_image=im,
                                    dark_image=im,
                                    size=(100, 100))
...
Lookupbutton = ctk.CTkButton(root, text="Whitelist information",fg_color="black",hover_color='grey', command = Lookup)
...
class WhiteListWindow(ctk.CTkToplevel):
    def __init__(self, *args, **kwargs):
        ...
        if key != '':
            ...
            Trialentro.pack()
            ScriptWhitelistbutton = ctk.CTkButton(self, text="Whitelist to script",fg_color="black",hover_color='grey', command= lambda: WhitelistScript(self,'SID'))
            ScriptWhitelistbutton.pack(pady=5)
            Whitelistbutton = ctk.CTkButton(self, text="Universal Whitelist",fg_color="black",hover_color='grey', command= lambda: WhitelistScript(self,'Universal'))
...
Whitelistbutton = ctk.CTkButton(root, text="Whitelist",fg_color="black",hover_color='grey', command = lambda: WhiteListWindow())
Whitelistbutton.pack()
...
Blacklistbutton = ctk.CTkButton(root, text="Blacklist",fg_color="black",hover_color='grey', command = Blacklist)
...
Blacklistbutton = ctk.CTkButton(root, text="Unblacklist",fg_color="black",hover_color='grey', command = Unblacklist)
...
Delwhitelistbutton = ctk.CTkButton(root, text="Delete Whitelist",fg_color="black",hover_color='grey',command=DelWhitelist)
...
RemoveCooldownbutton = ctk.CTkButton(root, text="Remove Cooldown",fg_color="black",hover_color='grey',command=RemoveCooldown)
...
ResetHIDbutton = ctk.CTkButton(root, text="Reset HWID",fg_color="black",hover_color='grey',command=ResetHWID)
ResetHIDbutton.pack(pady=5)
...
class KeyWindow(ctk.CTkToplevel):
    def __init__(self, *args, **kwargs):
        ...
        try:
            my_frame = KeysFrame(master=self,table_list = table_json, width=1900, height=850)
            my_frame.pack()
        except Exception as e:
...
Keysbutton = ctk.CTkButton(root, text="Keys",fg_color="black",hover_color='grey', command = KeyWindow)
Keysbutton.pack()

APIChangebutton = ctk.CTkButton(root, text="Change API Token",fg_color="black",hover_color='grey', command = API_event)
APIChangebutton.pack(pady=5)
